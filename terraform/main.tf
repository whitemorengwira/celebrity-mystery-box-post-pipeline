...
# Project: Celebrity Mystery Box (DStv · Mzansi Magic · Channel 161)
# ------------------------------------------------------------------------------
...
provider "aws" {
  ...
  default_tags {
    tags = {
      Project     = "Celebrity Mystery Box"
      Network     = "DStv · Mzansi Magic · Channel 161"
...
resource "aws_kms_key" "media_key" {
  description             = "KMS Key for Celebrity Mystery Box media asset encryption"
  deletion_window_in_days = 30
  enable_key_rotation     = true

  tags = {
    Name = "celebrity_mystery_box_post_pipeline-kms-key"
  }
}

# 2. S3 Media Vault Bucket
resource "aws_s3_bucket" "media_vault" {
  bucket        = "nwhite-celebrity-mystery-box-post-pipeline-${var.environment}"
  force_destroy = false
}
...
resource "aws_iam_role" "post_production_role" {
  name = "celebrity_mystery_box_post_pipeline-post-production-role"
...
resource "aws_iam_policy" "post_production_policy" {
  name        = "celebrity_mystery_box_post_pipeline-post-production-policy"
  description = "Scoped S3 and KMS permissions for Celebrity Mystery Box post-production workflow"
...
resource "aws_cloudwatch_log_group" "workflow_logs" {
  name              = "/aws/media-pipeline/celebrity_mystery_box_post_pipeline"
